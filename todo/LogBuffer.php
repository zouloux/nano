...
class LogBuffer
{
	 ...
	 * Will be stored into /var/logs/$category/$YYYY/$MM/$DD/$HH:$MM-$ID.log.
	 * @return false|int
	 */
...
//		];
		...
		// Unique filename
		$fileName = date("h:m")."-".uniqid().".log";
		// Convert buffer to string
		$buffer = $this->toString();
		// Inject to file and return successfulness
		return file_put_contents($directory."/".$fileName, $buffer);
	}
	...
	public function hasCritical () {
		...
		return $levels[LogBuffer::$LEVEL_ERROR] > 0 || $levels[LogBuffer::$LEVEL_CRITICAL] > 0;
	}
}
