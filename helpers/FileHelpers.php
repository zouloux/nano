...
class FileHelpers
{
	...
	static function listFolder ( string $path, bool $includeHidden = false ) {
		...
		foreach ( $files as $fileName ) {
			...
			if ( !$includeHidden && stripos($fileName, ".") === 0 ) continue;
			$f = new File();
	...
	static function recursiveSearchRoot ( string $rootPath, string $searchedFile, array $excludedFolders = [] ) {
		...
		foreach ( $files as $file ) {
			...
			if ( is_dir($filePath) ) {
				$r = FileHelpers::recursiveSearchRoot( $filePath, $searchedFile, $excludedFolders );
				if ( $r !== false ) return $r;
			}
		}
		return false;
	...
	static function recursiveRemoveDirectory ( string $path ) {
		...
		foreach ( $files as $file ) {
			...
			is_dir($filePath)
			? FileHelpers::recursiveRemoveDirectory( $filePath )
			: unlink( $filePath );
		}
		rmdir( $path );
	}
	...
	static function copyFolder ( string $from, string $to ) {
		...
		if ( is_dir($to) )
			FileHelpers::recursiveRemoveDirectory( $to );
